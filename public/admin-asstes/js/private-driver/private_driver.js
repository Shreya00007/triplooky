$(document).ready(function(){
	$(".select-region-of-private-driver").on("change",function(){
		$.ajax({
			type:"GET",
			url:"/admin/private-driver/get-city/"+$(this).val(),
			beforeSend:function(){
	...
	//add private driver
	$(".private-driver-form").submit(function(e){
		e.preventDefault();
		
		var editor_index=0;
		var editor=$(".private-driver-form .ql-editor");
	...
	var data=new FormData($(".private-driver-form")[0]);
	data.append("desc",$(".private-driver-form .desc .ql-editor").html());
	...
	data.append("_token",$("body").attr("data"));
		...
		if(input.length==input_index && select.length==select_index && editor.length==editor_index){
			...
			$.ajax({
				type:"POST",
				url:"/admin/driver/add-private-driver-data",
				data:data,
				contentType:false,
				processData:false,
				cache:false,
	...
	//end add private driver



	//update private driver
	$(".update-private-driver-form").submit(function(e){
		e.preventDefault();
		
		var editor_index=0;
		var editor=$(".update-private-driver-form .ql-editor");
		$(editor).each(function(){
			if($(this).hasClass("ql-blank")){
				var attr=$(this).parent().attr("data");
				if($(this).parent().parent().next().is("span")==false){
	...
	var data=new FormData($(".update-private-driver-form")[0]);
	data.append("desc",$(".update-private-driver-form .desc .ql-editor").html());
	...
	data.append("_token",$("body").attr("data"));
		...
		if(input.length==input_index && select.length==select_index && editor.length==editor_index){
			...
			$.ajax({
				type:"POST",
				url:"/admin/driver/update-private-driver-data",
				data:data,
				contentType:false,
				processData:false,
				cache:false,
				...
				window.scrollTo({ top: 0, behavior: 'smooth' });
					console.log(response);
					 if(response.message=="success"){
					 	...
					 	$(".message").html("<div class='alert alert-success animated flash infinite p-font'><b>Private Driver Updated Successfully created</b><i class='la la-close close' data-dismiss='alert'></i></div>");
					 	setTimeout(function(){
