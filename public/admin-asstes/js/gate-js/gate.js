$(document).ready(function(){
	$(".gate-form").submit(function(e){
		e.preventDefault();
		
		var editor_index=0;
		var editor=$(".gate-form .ql-editor");
	...
	var data=new FormData($(".gate-form")[0]);
	data.append("facility",$(".gate-form .desc .ql-editor").html());
	...
	$(".update-gate-form").submit(function(e){
		e.preventDefault();
		
		var editor_index=0;
		var editor=$(".update-gate-form .ql-editor");
		$(editor).each(function(){
			if($(this).hasClass("ql-blank")){
				var attr=$(this).parent().attr("data");
				if($(this).parent().parent().next().is("span")==false){
	...
	var data=new FormData($(".update-gate-form")[0]);
	data.append("facility",$(".update-gate-form .desc .ql-editor").html());
	...
	data.append("_token",$("body").attr("data"));
		...
		if(input.length==input_index && select.length==select_index && editor.length==editor_index){
			...
			$.ajax({
				type:"POST",
				url:"/admin/gates/update-gates-data",
				data:data,
				contentType:false,
				processData:false,
				cache:false,
				...
				window.scrollTo({ top: 0, behavior: 'smooth' });
					console.log(response);
					 if(response.message=="success"){
					 	...
					 	$(".message").html("<div class='alert alert-success animated flash infinite p-font'><b>Gates Updated Successfully created</b><i class='la la-close close' data-dismiss='alert'></i></div>");
					 	setTimeout(function(){
       
                   $(".message").html("");
                   $(".update-gate-form").trigger("reset");
